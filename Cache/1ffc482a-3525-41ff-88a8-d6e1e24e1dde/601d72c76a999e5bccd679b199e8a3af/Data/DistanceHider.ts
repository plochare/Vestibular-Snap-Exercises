...
export class DistanceHider extends BaseScriptComponent {
    ...
    private triggerParticleBurst(position: vec3) {
        ...
        try {
            ...
            const particleSystem = this.particleSystemPrefab.getComponent("ParticleSystem") as any;
    ...
    private disableParticleAfterDelay(delay: number) {
        try {
            ...
            const disableEvent = this.createEvent("UpdateEvent").bind(() => {
                if (getTime() - startTime >= delay) {
                    if (this.particleSystemPrefab) {
                        this.particleSystemPrefab.enabled = false;
                    }
                    disableEvent.unsubscribe();
                }
            });
        } catch (error) {
