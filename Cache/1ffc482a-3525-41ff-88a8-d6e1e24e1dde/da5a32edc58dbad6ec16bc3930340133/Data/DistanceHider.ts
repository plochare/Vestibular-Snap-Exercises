...
export class DistanceHider extends BaseScriptComponent {
    ...
    private playCollisionSound() {
        ...
        try {
            ...
            if (this.useRandomPitch) {
                const randomPitch = 1.0 + (Math.random() - 0.5) * this.pitchVariation * 2;
                this.collisionSoundEffect.setPitch(randomPitch);
            }
            
            // Play the sound effect
            this.collisionSoundEffect.play(PlaybackMode.Normal);
    ...
    public setUseRandomPitch(useRandom: boolean) {
        this.useRandomPitch = useRandom;
        this.log(`Random pitch: ${useRandom}`);
    }
    
    public setPitchVariation(variation: number) {
        this.pitchVariation = variation;
        this.log(`Pitch variation set to ${variation}`);
    }
}
