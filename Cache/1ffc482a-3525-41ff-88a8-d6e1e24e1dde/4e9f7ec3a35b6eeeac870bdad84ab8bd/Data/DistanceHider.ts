...
export class DistanceHider extends BaseScriptComponent {
    ...
    private hideObject(object: SceneObject, distance: number) {
        ...
        if (this.enableParticleBurst) {
            ...
            this.disableParticleAfterDelay(1.0);
        }
    ...
    private triggerParticleBurst(position: vec3) {
        ...
        try {
            ...
            const particleTransform = this.particleSystemPrefab.getTransform();
            particleTransform.setWorldPosition(position);
            
            // Enable particle system if disabled
            if (!this.particleSystemPrefab.enabled) {
                this.particleSystemPrefab.enabled = true;
            }
            
            // Play particle system using any type to bypass component type checking
            const particleSystemComponent = this.particleSystemPrefab.getComponent("Component.ParticleSystem") as any;
            if (particleSystemComponent && typeof particleSystemComponent.play === 'function') {
                particleSystemComponent.play();
                this.log(`✓ Particle burst triggered at (${position.x.toFixed(1)}, ${position.y.toFixed(1)}, ${position.z.toFixed(1)})`);
            } else {
                this.log(`WARNING: ParticleSystem component not found`);
            }
        } catch (error) {
    ...
    private disableParticleAfterDelay(delay: number) {
        try {
            const startTime = getTime();
            let updateEventRef: any = null;
            
            updateEventRef = this.createEvent("UpdateEvent").bind(() => {
                if (getTime() - startTime >= delay) {
                    if (this.particleSystemPrefab) {
                        this.particleSystemPrefab.enabled = false;
                    }
                    if (updateEventRef) {
                        updateEventRef.unsubscribe();
                    }
                }
            });
        } catch (error) {
            this.log(`ERROR: Failed to schedule particle disable`);
        }
    }
