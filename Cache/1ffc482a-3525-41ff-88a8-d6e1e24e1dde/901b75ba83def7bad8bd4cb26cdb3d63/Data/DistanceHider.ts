...
export class DistanceHider extends BaseScriptComponent {
    ...
    @input
    particleSystemObject: SceneObject;
    ...
    private particleDisableTimer: number = 0;
    private isParticleActive: boolean = false;
    ...
    onAwake() {
        ...
        if (!this.particleSystemObject) {
            this.log("WARNING: Particle system object is not assigned.");
        } else {
            this.log(`✓ Particle system object assigned`);
        }
    ...
    private onUpdate(eventData: UpdateEvent) {
        ...
        // Update particle disable timer
        if (this.isParticleActive) {
            this.particleDisableTimer -= deltaTime;
            if (this.particleDisableTimer <= 0) {
    ...
    private triggerParticleBurst(position: vec3) {
        ...
        try {
            // Move particle system to hide position
            const particleTransform = this.particleSystemObject.getTransform();
            particleTransform.setWorldPosition(position);
            
            // Enable particle system
            this.particleSystemObject.enabled = true;
            this.isParticleActive = true;
            this.particleDisableTimer = 1.0;  // Disable after 1 second
            
            this.log(`✓ Particle burst triggered at (${position.x.toFixed(1)}, ${position.y.toFixed(1)}, ${position.z.toFixed(1)})`);
        } catch (error) {
            this.log(`ERROR: Failed to trigger particle burst`);
        }
    }
    
    private disableParticleSystem() {
        if (this.particleSystemObject) {
            this.particleSystemObject.enabled = false;
            this.isParticleActive = false;
            this.log(`✓ Particle system disabled`);
        }
    }
    ...
    public setParticleSystemObject(particleSystem: SceneObject) {
        this.particleSystemObject = particleSystem;
        this.log(`Particle system object updated`);
    }
    ...
    public setParticleDisableDuration(duration: number) {
        this.particleDisableTimer = duration;
        this.log(`Particle disable duration set to ${duration} seconds`);
    }
}
