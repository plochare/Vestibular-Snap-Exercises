...
export class RowController extends BaseScriptComponent {
    ...
    @input
    useRandomPitchOnClear: boolean = true;
    
    @input
    clearPitchVariation: number = 0.15;
    ...
    private playRowClearedSound() {
        ...
        try {
            ...
            if (this.useRandomPitchOnClear) {
                const randomPitch = 1.0 + (Math.random() - 0.5) * this.clearPitchVariation * 2;
                this.rowClearedSoundEffect.setPitch(randomPitch);
            }
            
            // Play the sound effect
            this.rowClearedSoundEffect.play(PlaybackMode.Normal);
    ...
    public getUITextElement(): Text {
        return this.uiTextElement;
    }
    
    public setUITextElement(element: Text) {
        this.uiTextElement = element;
        this.log(`UI text element updated`);
    }
    
    public getRowTexts(): string[] {
        return this.rowTexts;
    }
    
    public setRowTexts(texts: string[]) {
        this.rowTexts = texts;
        this.log(`Row texts array updated (${texts.length} items)`);
    }
    
    public getRowText(rowIndex: number): string {
        if (rowIndex >= this.rowTexts.length) {
            return "";
        }
    ...
    public setRowText(rowIndex: number, text: string) {
        if (rowIndex >= this.rowTexts.length) {
            this.log(`ERROR: Row index ${rowIndex} out of bounds`);
            return;
        }
    ...
    public setUseRandomPitchOnClear(useRandom: boolean) {
        this.useRandomPitchOnClear = useRandom;
        this.log(`Random pitch on clear: ${useRandom}`);
    }
    
    public setClearPitchVariation(variation: number) {
        this.clearPitchVariation = variation;
        this.log(`Clear pitch variation set to ${variation}`);
    }
