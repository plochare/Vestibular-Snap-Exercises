@component
export class PinchBeginButton extends BaseScriptComponent {
    ...
    onAwake() {
        this.log("=== PinchBeginButton Initialization Started ===");
        ...
        this.log("=== PinchBeginButton Initialization Complete ===\n");
    }
    ...
    private onPinch(eventData: PinchEvent) {
        // Trigger on pinch begin (when user starts the gesture)
        if (eventData.state === GestureState.Began) {
            this.executeAction();
        }
    }
    ...
    private log(message: string) {
        if (this.enableDebug) {
            print(`[PinchBeginButton] ${message}`);
        }
    }
    ...
    public getStatus(): string {
        let status = "PinchBeginButton Status:\n";
